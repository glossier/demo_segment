view: page_facts {
  derived_table: {
    ...
    sql: SELECT e.event_id AS event_id
      , e.glossier_visitor_id
      , e.received_at
      , CASE
          WHEN DATEDIFF(seconds, e.received_at, LEAD(e.received_at) OVER(PARTITION BY e.glossier_visitor_id ORDER BY e.received_at)) > 30*60 THEN NULL
          ELSE DATEDIFF(seconds, e.received_at, LEAD(e.received_at) OVER(PARTITION BY e.glossier_visitor_id ORDER BY e.received_at)) END AS lead_idle_time_condition
